...
const app = createApp();
const schema = z.object({ siteName: z.string().max(200).optional(), siteDescription: z.string().max(500).optional(), logoUrl: z.string().max(500).optional(), instagramUrl: z.string().url().optional(), baseSeoTitle: z.string().max(200).optional(), baseSeoDescription: z.string().max(500).optional(), featuredPostId: z.coerce.number().optional(), homepagePostCount: z.coerce.number().min(1).max(50).default(12) });
export async function getPublicSettings(db: ReturnType<typeof createDb>) {
  const rows = await db.select().from(settings); const map = new Map(rows.map(r => [r.key, r.value]));
  return { siteName: map.get("site_name") || "iAtomic", siteDescription: map.get("site_description") || "مجله علمی آیاتمیک", logoUrl: map.get("logo_url") || null, instagramUrl: map.get("instagram_url") || "https://instagram.com/iatomic_", baseSeoTitle: map.get("base_seo_title") || "iAtomic Magazine", baseSeoDescription: map.get("base_seo_description") || "", featuredPostId: map.get("featured_post_id") ? parseInt(map.get("featured_post_id")!, 10) : null, homepagePostCount: parseInt(map.get("homepage_post_count") || "12", 10) };
}
...
app.put("/", requireAuth, zValidator("json", schema), async (c) => {
  const body = c.req.valid("json"); const db = createDb(c.env.DB); const entries: { key: string; value: string }[] = [];
