...
const $=s=>document.querySelector(s);const api=(p,o={})=>fetch(p,{credentials:'include',headers:{'content-type':'application/json'},...o}).then(async r=>{if(!r.ok)throw new Error((await r.json().catch(()=>({error:r.statusText}))).error);return r.json()});let configs=[],subToken='';
...
async function copyAll(){let r=await api('/api/export?format=links');navigator.clipboard.writeText(r.text);alert('کپی شد')}async function exportClash(){let r=await api('/api/export?format=clash');view.innerHTML='<section class="card"><h2>Clash Meta</h2><pre class="output">'+r.text+'</pre><button class="btn" onclick="navigator.clipboard.writeText(document.querySelector(\'.output\').textContent)">کپی</button></section>'}async function exportSingbox(){let r=await api('/api/export?format=singbox');view.innerHTML='<section class="card"><h2>Sing-box</h2><pre class="output">'+r.text+'</pre><button class="btn" onclick="navigator.clipboard.writeText(document.querySelector(\'.output\').textContent)">کپی</button></section>'}
